 ...
 */
public class GeneratorShortCastling implements IGeneratorMove {
    ...
    public List<Move> getMove(CellBoard currentCell, TypeGamer typeGamer) {
      ...
      //  Move move;
        ...
        if (typeGamer == TypeGamer.B) {

            iterator = currentCell.getIterator(AdjacentCell.Left);
        }
        else if (typeGamer == TypeGamer.W)
        {
            iterator = currentCell.getIterator(AdjacentCell.Right);
        }
        else
        {
            return null;
  ...
  /*  protected Move createMove(CellBoard currentCell, CellBoard next) {
    ...
    protected void castleMove(CellBoard currentCell, CellBoard next) {
        result.add(new Castling (currentCell, next, next.getRight(), currentCell));
